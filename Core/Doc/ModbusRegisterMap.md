...
### 2.3 时间与调度区 `0x002E~0x0050`

| 地址     | 符号                       | 说明 |
|----------|----------------------------|------|
| `0x002E` | `REG_HOLD_TIME_HHMM`       | 当前时间 = 小时×100 + 分钟（如 1435 表示 14:35）；写入=授时；HMI 地址 40047 |
| `0x002F` | `REG_HOLD_TIME_DOW`        | 星期（0=周日, 1=周一, ..., 6=周六）；HMI 地址 40048 |
...
**智能模式关闭逻辑：** 在 `REG_HOLD_MODE=1` 且时间已同步后，命中调度段时切换到该段场景；未命中任何使能段时切换到场景 5（默认全关）。
...
```
写 0x002E = 1435        ← 授时 14:35（HMI 40047）
写 0x002F = 3           ← 周三（HMI 40048）
...
### 6.4 单组定时开关灯（MCGS/HMI 常用）

```text
写 40047 = 当前 HHMM    ← 例如 1435
写 40048 = 当前星期    ← 0=周日, 1=周一, ..., 6=周六
写 40049 = 开灯 HHMM    ← 段 0 起始时间
写 40050 = 关灯 HHMM    ← 段 0 结束时间
写 40051 = 1           ← 段 0 场景 1：全开
写 40052 = 0x80        ← 每天生效
写 40081 = 0x01        ← 启用段 0
写 40001 = 1           ← 切到智能模式
```
